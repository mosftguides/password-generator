<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Password Generator</title>
    <link rel="stylesheet" href="style.css">
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Open+Sans:ital,wght@0,300;0,400;0,500;0,600;1,300;1,400;1,500;1,600&display=swap" rel="stylesheet">
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Roboto:ital,wght@0,100;0,300;0,400;0,500;0,700;0,900;1,100;1,300;1,400;1,500;1,700;1,900&display=swap" rel="stylesheet">
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Ubuntu:ital,wght@0,300;0,400;0,500;0,700;1,300;1,400;1,500;1,700&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="style.css">
</head>
<body>
    <?php
    # Daten für die spätere Nutzung vorbereiten
    error_reporting(0); # keine Fehler anzeigen
    # von index.php gesendete Daten in Variablen schreiben
    $len = $_POST['length'];
    $qua = $_POST['quanity'];
    $num = $_POST['num'];
    $son = $_POST['son']; 

    if ($son=="sonY"){
        # uppercase letters, lowercase letters and special characters
        $chars='abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ!$%&/\()=?{[]}';
    }
    else {
    # only uppercase letters and lowercase letters
    $chars = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
    }

?>
    <!-- the website's headings -->
    <h1 class="opensans">Password Generator</h1>
    <h2 class="ubuntu">easy to use, fast and available for all devices and platforms</h2>
    <h2 class="roboto">Your Password(s) </h2>
<?php
    # generating the password(s)
    for ($i = 1; $i <= $qua; $i++) {
        echo '<br>';
        $ranstr = substr(str_shuffle(str_repeat($chars, ceil($len/strlen($chars)) )),0,$len);
        echo '<h3 class="roboto">' . $ranstr . '</h3>';
    }


    # specification of the programming languages used for this and their logos
    echo '<br>';
    echo '<p class="roboto">For this website, the programming languages PHP and HTML were used.<p>';
    echo "<br><br>";

    # Bilder
    ?>
    <img class="ohpimg2" src="php-logo.png"></img>
    <img class="ohpimg1" src="html-logo.png"></img>
</body>
</html>